"""
chatkhu 일별 멤버 사용량 스냅샷 다운로드 스크립트

날짜별로 /api/proxy/admin/members/export-task/ 를 호출해
member_YYYY-MM-DD.csv 파일을 생성합니다.

사용법:
    python3 -u code/download_daily.py
...
OUTPUT_DIR    = "/home/user/workspace/chatkhu/usage_data_deidentification"
...
def main():
    ...
    total = len(days)
    print(f"총 {total}일 ({START_DATE} ~ {END_DATE}) 멤버 스냅샷 다운로드 시작", flush=True)
